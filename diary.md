...
そういえば手元とジャッジの実行速度揃えてなかった。明日ちゃんとやります……。

[高速な倍精度指数関数expの実装](https://www.slideshare.net/herumi/exp-9499790) ←「exp cpuクロック」でググったら出てきた。もしかしたら使うかも。

## 7日目（2/15）

昨日出したアイデアについて考える。[AHC003のeivourさんの記事](https://qiita.com/contramundum/items/b945400b81536df42d1a)を参考にしている。

各マスの石油埋蔵量が多変数正規分布にしたがうと大胆にも仮定すると、記事の正規分布モデルの議論を流用できる。事前分布の期待値ベクトルμと共分散行列Σはランダムに油田の配置を大量生成することで近似的に求められる。毎ターンクエリを投げると石油埋蔵量の測定値が返ってくるので、これもクリッピング・丸めのない正規分布と見なしてあげれば記事の通りμとΣの更新が行える。

このとき、油田の左上マスが(r, c)に置かれていることがどの程度あり得そうかを考える。マス(r + i, c + j)に油田が埋まっている場合、そのマスの埋蔵量は1以上でなければならない。したがって、油田の左上マスが(r, c)に置かれている尤度を知るためにはマス(r + i0, c + j0), (r + i1, c + j1), ...の埋蔵量が全て1以上である確率を求めればよい。多変数正規分布の積分を高速に行う方法を知らないので大胆にも埋蔵量の共分散が0と仮定すると、1次元正規分布のx≧0.5の領域の積分値を求めて掛け合わせてあげればそれっぽくなるはず。あとはこれを尤度の降順にソートして確率を正規化し、累積確率が閾値を超えたらそれ以降の置き場所はほぼあり得ないとして枝刈りしてあげればよい。確率論は得意じゃないので所々怪しい気もするが、枝刈りに使うだけだし怪しければ閾値を緩和すれば何とかなるだろう。もしこれで油田の置き場所の候補を1/Kに絞れれば探索空間は(1/K)^M（現在使用しているm個部分破壊近傍の大きさは(1/K)^m）に減らせるのでかなり嬉しい気がする。後半になるほど候補数が減るはずというのも相性が良い。

実装したらスコアが下がった……。正解のところまで枝刈りしてしまっている。何か間違えていそう。

なんとなく分かってきたかも。無相関を仮定しているため、マップの端の方となるとマスの集合が全て1以上の埋蔵量を「たまたま」持っている確率はかなり低くなる。やっぱりちゃんと油田の形状を考えないとダメ。

ある観測について、油田iをある場所に存在する尤度を考えてみる。極端な例を考えると、油田iと同じマス集合を観測したのに観測値が0となる確率はかなり低いはず。|油田マス集合∩観測マス集合|の個数がxのとき、真の値tはx以上でなければならない。よって真の値がx以上になる確率を累積和などを使って求めて掛け合わせて、尤度の低い位置を枝刈りすれば良い。これならどうだ？

計算量的に間に合うかという点をいったん置いておくと、もう少し議論が進められる。例えば油田が10個あったとして、ある計測でv=5と出たときに1個目の油田で|油田マス集合∩観測マス集合|=5となるケースと、|油田マス集合∩観測マス集合|=0となるケースとでは、後者の方が起こりやすそう。これを考えるには、油田i以外についての|油田マス集合∩観測マス集合|の和がyになる確率を考えれば良くて、これは左右からDPをすることで正確に求められる。計算量的に余裕があればこの確率P(other=y)を使って、P(v|x)=Σ_y P(v|x,y)P(y|x) = Σ_y P(v|x,y)P(y) を計算すれば良い？計算量的に厳しければ、yの最小値y_minと最大値y_maxだけ求めておいて、真の値tがx+y_min以上x+y_max以下である確率を求めればよい。これは予め累積和を求めておくことでO(1)で求められる。

実装してみたが……うーん、微妙。若干悪化と言ったところ。後者のやつも実装してみるが、これは必要な候補まで消してしまう。うーんダメだ……。しばらく考えてみたもののよく分からず。尤度とか周辺確率とかをちゃんと理解できていない気がする。有給一日使っただけにダメージが大きい。

テンション下がりながらサンプリング候補山登りの初期解をいじいじしてたらなんか結構上がった。しかも偶然実装ミスったやつが当たった。人生よくわからんものだ……。

あとエントロピー計算時に見てあげる候補数を増やしたり、簡単なケースが一瞬で終わってしまっていたのでもう少し時間を伸ばすように調整したり、エントロピー計算で自動ベクトル化が効きやすいコードに変えたりした。最後の詰めに取っておくべきものを食い潰しているようで微妙だが、ひとまず2位に上がれたので良かった。
...
- サンプリング候補山登りの初期解をちゃんと考察する
- サンプリング候補山登りはスコア計算が重いので大きめの近傍が作れないか考える
  - ……と書いたが、本当にスコア計算が重いのかも要検証
- MCMCもどきの近傍を考える（ある座標の油田を全て爆破して再配置とか？連結成分BFSは上手く行かなかったが）
- 近傍計算が高速に行えないか考える
- お手つきが多いのをなんとかする
- epsが大きいときは同じ場所を2連打して分散を小さくしつつ計算コスト（計測回数に比例）を抑える
- サンプリング候補山登りの後処理で、計測結果に全く関係のない箇所を埋めるべきか消すべきか考察
- 正規分布の累積分布関数のより良い数値計算がないか検討（ロジスティック分布は裾が厚いはず）
